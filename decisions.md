...
- **Decision:** Semantic Rename Detection.
- **Reasoning:** To prevent data loss during column renames, we use a heuristic that identifies a rename when exactly one column is dropped and one is added with identical semantic properties (Type, Nullability, Auto-Increment). This allows us to use `RENAME COLUMN` instead of `DROP` + `ADD`.
...
- **Decision:** SQL Identifier Safety.
- **Reasoning:** All table, column, and index names are passed through a centralized quoting utility (`safeIdentifier`) to prevent SQL injection and handle identifiers containing reserved words or special characters.
...
## Concurrency Control
- **Decision:** State-based Locking for `/apply`.
- **Reasoning:** To prevent simultaneous migration requests on the same project, we use the `Project.status` field as a lock. A request is rejected with `409 Conflict` if the project is not in the `READY` state.
...
- **Advanced Distributed Locking**: Omitted distributed locking (e.g., Redis) for `/apply` in favor of a simpler state-based lock in the primary DB.
